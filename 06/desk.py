    ...
    def get_status(self, timestamp):
        for time_range in self.time_ranges:
            if time_range.start <= timestamp < time_range.stop:
                return f"{self.owner} got desk until {time_range.stop}"
            elif timestamp < time_range.start:
                return f"desk available unit {time_range.start}"

        return "desk is available"
